...
<section id="insideContent">
    ...
    <div class="tipsList">
            ...
            <?php   foreach($this->maisDicas as $dica){
              ...
              ?>
                <li>
                    <div class="photo" style="background-image: url(<?= $fto->getFile()->getPath2();?>);"><span><?=date("d", strtotime($dica->CNT_DTA))?><br /> {<?= "Month: ".date("m", strtotime($dica->CNT_DTA))?>}</span><a href="<?=$dica->getURL();?>" class="linkImg"></a></div>
                    <div class="description">
                        <div class="alignDescription">
                            <h3><?=$dica->CNT_TIT?></h3>
                            <div><?=nl2br($dica->CNT_RES)?></div>
  ...
  ?>
              <div class="txtCenter"><a href="<?php echo($this->site->PDT_URL); ?>maisDicasnutricao?titulo=<?=$this->titulo?>" class="button ajaxed" data-target=".tipsList ul" data-posFunc="pagerDoido">{Carregar mais}</a></div>
          <?php
          }else{
              ?>
              <div class="txtCenter"><a href="<?php echo($this->site->PDT_URL); ?>maisDicasnutricao?page=1" class="button ajaxed" data-target=".tipsList ul" data-posFunc="pagerDoido">{Carregar mais}</a></div>
          <?php
          }
          ?>
            <?php } else { ?>
