			...
			<div class="col-1">
				...
				<div class="box">
					...
					<div>
						...
						?>
						<p><a href="<?php printf("%spagina/%s/%s.html", $this->Site->getPDTURL(), date("Y/m/d", strtotime($dta)), $url); ?>" class="linkSite" target="_blank">
						<?php printf("%spagina/%s/%s.html", $this->Site->getPDTURL(), date("Y/m/d", strtotime($dta)), $url); ?>
						</a></p>
						<!-- /LinkSite -->
						<?php } ?>
						
						<label for="cnt_olh" class="hidden">{Intro}</label>
						<input type="text" name="news[CNT_OLH]" id="cnt_olh" class="inputText" placeholder="Olho" value="<?php echo($this->content->getCNTOLH()); ?>"/><br />
						...
						<label for="cnt_emb">Template</label>
						<input type="text" name="news[CNT_EMB]" value="<?php echo($this->content->getCNTEMB()); ?>" /><br/>
						
						<label for="cnt_loc">Classe</label>
						<input type="text" name="news[CNT_LOC]" value="<?php echo($this->content->getCNTLOC()); ?>" />
				...
				<div class="box">
					<h3>{SEO}</h3>
					<div>
						<label for="cnt_rdt" class="small-label rightTxt">{Title}</label> <input type="text" name="news[CNT_RDT]" id="cnt_rdt" value="<?php echo($this->content->getCNTRDT()); ?>" /> <br />
						
						<label for="cnt_ema" class="small-label  rightTxt">{Keys}</label> <input type="text" name="news[CNT_EMA]" id="cnt_ema" value="<?php echo($this->content->getCNTEMA()); ?>"/> <br />
						
						<label for="cnt_res" class="no-points"><strong>{Description}</strong></label><br />
						<textarea name="news[CNT_RES]"><?php echo($this->content->getCNTRES()); ?></textarea>
					</div>
				</div>
				<!-- /SEO Block -->
				<!-- /Conteudo -->
				<?php //$this->insertBlock("home", "linksBox", array("CONTAINER" => "news", "CONTENT" => $this->content)); ?>
