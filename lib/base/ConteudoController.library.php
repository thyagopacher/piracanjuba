<?php
	 abstract class ConteudoController extends DefaultBackEnd2Controller
	{
		const THUMB_TYPE = "THB_CNT";
    public $dbAbbreviation = "NT";
		public $moduleName = "Notícias";
		public $moduleDir = "news";
		public $editorialTaxonomy = "CAT_EDT";
		...
		public function index($vars = null)
		{
			...
			$this->breadCrumb = array("{$this->moduleDir}/index.php" => "{News}");

			$criteria = new FuriousSelectCriteria();
			$criteria->add("`cnt_conteudo`.`CNT_TIP`", $this->dbAbbreviation, FuriousExpressionsDB::EQUAL);
			...
			$this->doPagination("ConteudoModel", "itens", $criteria, $vars);
		}
		public function novo($vars = null)
		{
			...
			if($newsID != null)
			{
				$this->breadCrumb["{$this->moduleDir}/new.php"] = "{Edit News}";
				$this->pageTitle = "{Edit News}";
				$content = ConteudoModel::getOne(addslashes($newsID));
				$this->content = $content[0];


			} else {
				$this->breadCrumb["{$this->moduleDir}/new.php"] = "{Add News}";

				$this->content = new Conteudo();
				$this->content->CNT_SIT  = $this->Site->getPDTID();
				$this->content->CNT_IPR  = $this->EditorialID;
				$this->content->CNT_TIP = $this->dbAbbreviation;
			...
			if(Dispatcher::getPostValues("news"))
			{
				...
				if(!empty($edts[0]))
				{
					...
					$this->updateEdtsRelations($ID, $edts, $this->dbAbbreviation);
				}
				if(!empty($tags[0]))
				{
		...
		public function delete($vars = null)
		{
			...
			if(!empty($_GET["ID"]))
			{
				$content = ConteudoModel::getOne(addslashes($_GET["ID"]));
				$this->content = $content[0];
