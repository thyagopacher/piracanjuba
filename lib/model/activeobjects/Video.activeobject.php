<?php
	class Video extends Conteudo {
		protected $activeModel = "VideoModel";
		...
		public function generateTagsJsonList()
		{
			...
			if(!empty($this->tags[0]))
			{
				...
				foreach($this->tags as $tag)
				{
					...
					$news = VideoModel::doSelect($criteria);
		...
		public function genereateEditorialListFile($edts, $siteID)
		{
			...
			//$criteria->add("`cnt_categorias_conteudos`.`CCL_TIP`", "VD", FuriousExpressionsDB::EQUAL);
		...
		public function generateSiteList()
		{
			...
			Document::writeFile(APP_JSON_PATH."SITE_{$siteID}_VD.json", json_encode($t));
		}
